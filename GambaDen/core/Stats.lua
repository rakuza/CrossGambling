function GambaDen:Copy_Table(src, dest)
    --Adding the old stats to the new table. 
	for index, value in pairs(GambaDen["stats"]) do
...
function GambaDen:reportStats(full)
...
function GambaDen:updatePlayerStat(playerName, amount)
...
function GambaDen:joinStats(info, args)
...
function GambaDen:unjoinStats(info, altname)
...
function GambaDen:listAlts(info, mainname, altname)
    -- Alt List
...
function GambaDen:updateStat(info, args)
...
function GambaDen:deleteStat(info, player)
...
function GambaDen:resetStats(info)
