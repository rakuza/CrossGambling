GambaDen = LibStub("AceAddon-3.0"):NewAddon("GambaDen", "AceConsole-3.0", "AceEvent-3.0")
local GambaDen = LibStub("AceAddon-3.0"):GetAddon("GambaDen")
...
function GambaDen:OnInitialize()
	self:InitDB()
	self:InitMinimap()
...
function GambaDen:InitDB()
    ...
    GDCall = {}
	self.db = LibStub("AceDB-3.0"):New("GambaDen", defaults, true)
	LibStub("AceConfig-3.0"):RegisterOptionsTable("GambaDen", options, {"GambaDen", "gd"})
	if(GambaDen["stats"]) then GambaDen["stats"] = self.db.global.stats end
	 self.game.chatframeOption = true
end

function GambaDen:InitMinimap()
	local minimapIcon = LibStub("LibDBIcon-1.0")
	local minimapLDB = LibStub("LibDataBroker-1.1"):NewDataObject("GambaDenIcon", {
	type = "data source",
	text = "GambaDen",
	icon = "Interface\\AddOns\\GambaDen\\media\\icon",
	OnClick = function()
	...
	tooltip:AddLine("GambaDen", 1, 1, 1)
	tooltip:AddLine(" ", 1, 1, 1)
	tooltip:AddLine("Toggle GambaDen.", 1, 229 / 255, 153/ 255)
	tooltip:Show()
end,
})
minimapIcon:Register("GambaDenIcon", minimapLDB, self.db.global.minimap)

function GambaDen:ToggleMinimap()
	if (self.db.global.minimap.hide == false) then
		minimapIcon:Hide("GambaDenIcon")
		self.db.global.minimap.hide = true
	else
		minimapIcon:Show("GambaDenIcon")
...
function GambaDen:ToggleGUI(info, isShowing)
	local method = isShowing and "Show" or "Hide"
	local theme = self.db.global.theme

	if theme == uiThemes[1] then
		GambaDen[method .. "Classic"](info)
	elseif theme == uiThemes[2] then
		GambaDen[method .. "Slick"](info)
	end
end

function GambaDen:SendMsg(event, arg1)
  local msg = event
  if arg1 then msg = msg .. ":" .. tostring(arg1) end
  if self.game.chatMethod then
    ChatThrottleLib:SendAddonMessage("BULK", "GambaDen", msg, self.game.chatMethod)
  end
end

function GambaDen:RegisterChatEvents()
...
function GambaDen:handleChatMsg(_, text, playerName)
...
function GambaDen:handleSystemMessage(_, text)
...
function GambaDen:getPlayerByName(name)
...
function GambaDen:hasPendingRolls()
    for i = 1, #self.game.players do
...
function GambaDen:banPlayer(info, playerName)
...
function GambaDen:unbanPlayer(info, playerName)
    if not playerName or playerName == "" then
        self:Print("Error: No name provided.")
        return
    end
...
function GambaDen:listBans(info)
    ...
    if #bans == 0 then
        ...
        self:Print("The Current Bans, to unban use /gd unban [PlayerName]")
    end
end


function GambaDen:changeGameMode()
	local gameNum
...
function GambaDen:CheckRolls(playerName)
    local playersRoll = {}
    for i = 1, #self.game.players do
...
function GambaDen:registerPlayer(playerName, playerRoll)
    for i = 1, #self.game.players do
...
function GambaDen:unregisterPlayer(playerName)
...
function GambaDen:UnRegisterChatEvents()
